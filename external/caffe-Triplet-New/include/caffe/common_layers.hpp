...
template <typename Dtype>
class PSLayer : public Layer<Dtype> {
 public:
  explicit PSLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "PS"; }
  virtual inline int MinBottomBlobs() const { return 1; }
 ...
 protected:
  ...
  int bin_w_,bin_h_,bin_h_4_,num_,channels_,bottom_w_,bottom_h_;
};
