...
template <typename Dtype>
class SiameseContrastiveLossLayer : public LossLayer<Dtype> {
 public:
  explicit SiameseContrastiveLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), diff_() {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline int ExactNumBottomBlobs() const { return 3; }
  virtual inline const char* type() const { return "SiameseContrastiveLoss"; }
  //Dtype lamda;
  /**
   * Unlike most loss layers, in the ContrastiveLossLayer we can backpropagate
   * to the first two inputs.
 ...
 protected:
  ...
  Blob<Dtype> diff_;        // cached for backward pass
  Blob<Dtype> dist_sq_ab_;  // cached for forward pass
  Blob<Dtype> dist_sq_pp_;  // cached for forward pass
};
...
/*  virtual void Backward_gpu(const vector<Blob<Dtype>*>& top,  
  ...
  Blob<Dtype> diff_an1_;  // cached for backward pass  
  Blob<Dtype> diff_pn1_;  // cached for backward pass  
  Blob<Dtype> diff_w_;  // cached for backward pass  
  ...
  Blob<Dtype> dist_sq_pn_;  // cached for backward pass

  Blob<Dtype> dist_a_;      // cached for backward pass
  Blob<Dtype> dist_p_;      // cached for backward pass 
  Blob<Dtype> dist_n_;      // cached for backward pass  
  Blob<Dtype> dist_aN_;      // cached for backward pass
  Blob<Dtype> dist_pN_;      // cached for backward pass 
...
template <typename Dtype>  
class TripletContrastiveLossLayer : public LossLayer<Dtype> {  
 public:  
  explicit TripletContrastiveLossLayer(const LayerParameter& param)  
      : LossLayer<Dtype>(param){}  
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,  
      const vector<Blob<Dtype>*>& top);  
  
  virtual inline int ExactNumBottomBlobs() const { return 4; }  
  virtual inline const char* type() const { return "TripletContrastiveLoss"; }  
  /** 
   * Unlike most loss layers, in the TripletLossLayer we can backpropagate 
   * to the first three inputs. 
   */  
 ...
 //    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);  
  ...
  Blob<Dtype> diff_c_ap_;  // cached contrastive loss for backward pass  
  Blob<Dtype> diff_c_an_;  // cached contrastive loss for backward pass  
  Blob<Dtype> diff_c_pn_;  // cached contrastive loss for backward pass  
  
  Blob<Dtype> diff_sq_ap_;  // cached for backward pass  
  Blob<Dtype> diff_sq_an_;  // tmp storage for gpu forward pass 

  Blob<Dtype> diff_temp1_;   //cache temp diff 
  Blob<Dtype> diff_temp2_;   //cache tepm diff
